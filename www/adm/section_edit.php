...
$l = $thispg = $thumb = null;
if(isset($_POST["submit"]) && isset($_POST["id_SEC"]))
{
	$errMSG=check_empty_fields($_POST,$arrTextes);
	if($errMSG=="")
	{
		settype($_POST["id_SEC"],"INT");
		...
		try
		{
			...
			{	
				$strSQL = "INSERT INTO ".PREFIX."_section_text (id_SEC,lan,name,texto,web_title,meta_key,meta_des) 
							VALUES (".$_POST["id_SEC"].",'$value','".$objConn->prepMysql($_POST[$value."_name"])."','".$objConn->prepMysql($_POST[$value."_txt"])."','".$objConn->prepMysql($_POST[$value."_web_title"])."','".$objConn->prepMysql($_POST[$value."_meta_key"])."','".$objConn->prepMysql($_POST[$value."_meta_des"])."')
...
else
{
	if(isset($_GET["id_SEC"]))
	{
		...
		try
		{
			...
			{
				$strSQL = "UPDATE ".PREFIX."_section SET thumb='".$_GET["thmb"]."' WHERE id_SEC=".$_GET["id_SEC"]."";
				$objConn->i_query($strSQL);
			}
            if(isset($_GET["akt"]) && $_GET["akt"]=="remthumb") // aktion from media page _ remove thumbnail information 
			{
				$strSQL = "UPDATE ".PREFIX."_section SET thumb='' WHERE id_SEC=".$_GET["id_SEC"]."";
				$objConn->i_query($strSQL);
			}
		
			$strSQL = "SELECT lan.lan, lan.name AS lname,lan.notext AS lnotext,meta.meta_key, meta.meta_des,
					(SELECT gln.name FROM ".PREFIX."_section_text AS gln WHERE gln.lan=lan.lan AND gln.id_SEC=".$_GET["id_SEC"].") AS gname,
					(SELECT gln.texto FROM ".PREFIX."_section_text AS gln WHERE gln.lan=lan.lan AND gln.id_SEC=".$_GET["id_SEC"].") AS gtexto,
					(SELECT gln.web_title FROM ".PREFIX."_section_text AS gln WHERE gln.lan=lan.lan AND gln.id_SEC=".$_GET["id_SEC"].") AS web_title,
					(SELECT gln.meta_key FROM ".PREFIX."_section_text AS gln WHERE gln.lan=lan.lan AND gln.id_SEC=".$_GET["id_SEC"].") AS meta_key,
					(SELECT gln.meta_des FROM ".PREFIX."_section_text AS gln WHERE gln.lan=lan.lan AND gln.id_SEC=".$_GET["id_SEC"].") AS meta_des,
                    (SELECT thumb FROM ".PREFIX."_section WHERE id_SEC=".$_GET["id_SEC"].") AS thumb
	...
	<div id="mainpage">
		...
		<div class="form_two_columns">
			...
			<form action="<?=$_SERVER['PHP_SELF']?>" method="post" name="ngall" id="ngall">
				<input type="hidden" name="id_SEC" id="id_SEC" value="<?php if(isset($_REQUEST["id_SEC"])) echo $_REQUEST["id_SEC"]?>">
				<div id="accordion">
                <?php
				...
				{
					if($key!="submit" && $key!="id_SEC" && $key!="thumb")
					{
						$arrV=explode("_",$key);
						if($l!=$arrV[0])
						{
				...
				<ul class="form">
                    ...
                    <li>
						...
						if ($rs->count() > 0)
						{
							$querystring = $_SERVER["PHP_SELF"]."?id_SEC=".$_REQUEST["id_SEC"]."&name=".$_POST[$cmslan."_name"]."&id_ZONA=".$_REQUEST["id_ZONA"];
							while($img = $rs->fetchObject())
							{
                                ...
                                echo "<a href=\"".$_SERVER['PHP_SELF']."?id_SEC=".$_GET["id_SEC"]."&akt=remthumb&pg=$thispg&thmb=$img->url\" class=\"aktlink\" id=\"glthumb,".$_GET["id_SEC"].",$img->url\"><i class=\"fa fa-home $icon fa-lg fa-fw\" aria-label=\"$msg\"></i></a>";
								echo "</div>";
							}
							echo "<br class=\"break\"/>";
							echo "</div>";
					...
					<li>
						...
						$strSQL = "SELECT med.id_MED, med.tipo, med.url
								  FROM ".PREFIX."_media AS med
								  JOIN ".PREFIX."_section_media AS medn ON medn.id_MED=med.id_MED WHERE medn.id_SEC='".$_REQUEST["id_SEC"]."'
						...
						if ($rs->count() > 0)
						{
							...
							while($img = $rs->fetchObject())
							{
								...
								echo "<a href=\"$querystring&id_MED=$img->id_MED&isH=$isH\" class=\"imghref_ispag\" id=\"$img->id_MED,$isH,".$_REQUEST["id_SEC"].",id_SEC,section\"><i class=\"fa $icon fa-lg fa-fw\" aria-label=\"$msg\"></i></a>";
								echo "</div>";
							}
							echo "<br class=\"break\"/>";
							echo "</div>";
