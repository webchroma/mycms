	...
	<div id="mainpage">
			...
			<table width="100%">
				...
				if(isset($_POST["cpos"]) && isset($_POST["pos"]))
				{
					...
					{
						$strSQL = "UPDATE ".PREFIX."_land SET pos=".$value." WHERE id_LAND=".$key."";
						$objConn->i_query($strSQL);
					}
				}
				
				if(isset($_GET["id_LAND"]) && isset($_GET["akt"]))
				{
					...
					if($_GET["akt"]=="dea")
					{
						if(isset($_GET["isA"]))
						{
							...
							$strSQL = "UPDATE ".PREFIX."_land SET aktiv=".$_GET["isA"]." WHERE id_LAND=".$_GET["id_LAND"]."";
							$objConn->i_query($strSQL);
					...
					{
						$strSQL = "DELETE FROM ".PREFIX."_land WHERE id_LAND=".$_GET["id_LAND"]."";
						$objConn->i_query($strSQL);
				...
				$strSQL = "SELECT lnd.id_LAND AS rowID, lnd.pos, lndn.name, lnd.aktiv,
                            (SELECT GROUP_CONCAT(ID_SEC) FROM ".PREFIX."_land_section WHERE id_LAND=rowID) AS id_SECS
				...
				if ($rs->count() > 0)
				{
					...
					while($row = $rs->fetchObject())
					{
						...
						if(($iID!=$row->rowID) && $startrope)
							doActions($iID,$iname,$isA,false,false,true,false,"id_LAND",false,false,false);
						if($iID!=$row->rowID)
						{
							...
							echo "<td class=\"td2\">";
							echo "<a href=\"".THISMAINPAGENAME."_edit.php?id_LAND=$row->rowID&name=$row->name\">$row->name</a>";
							echo "</td>";
                            echo "<td class=\"td5\">";
