...
if(isset($_POST["submit"]))
{
	...
	if($errMSG=="")
	{
		if(!isset($_POST["ongoing"]))
			$_POST["ongoing"] = 0;
		$strSQL = "INSERT INTO ".PREFIX."_project (aktiv,ongoing,symlink,pos) SELECT 0,1,'".$objConn->prepMysql(strtolower($_POST["symlink"]))."',IFNULL(MAX(pos)+1,1) FROM ".PREFIX."_project";
		try
		{
			$objConn->stopCOMMIT();
			if($objConn->i_query($strSQL))
			{
				$iID = $objConn->getInsertID();
				foreach($arrL AS $value)
				{
					$strSQL = "INSERT INTO ".PREFIX."_project_text (id_PROJ,lan,name,texto,web_title,meta_key,meta_des) VALUES ($iID,'$value','".$objConn->prepMysql($_POST[$value."_name"])."','".$objConn->prepMysql($_POST[$value."_txt"])."','".$objConn->prepMysql($_POST[$value."_web_title"])."','".$objConn->prepMysql($_POST[$value."_meta_key"])."','".$objConn->prepMysql($_POST[$value."_meta_des"])."')";
				...
				header("location:".THISMAINPAGENAME."_edit.php?id_PROJ=$iID");
			}
			else
			{
			...
			<form action="<?=$_SERVER['PHP_SELF']?>" method="post" name="nnews" id="nnews">
				<ul class="form">
					<li>
						...
						<br />
						<input type="text" name="symlink" id="symlink" value="" size="50" maxlength="50" />
						<br />
