...
if(isset($_POST["submit"]))
{
	...
	if($errMSG=="")
	{
		$strSQL = "INSERT INTO ".PREFIX."_land (aktiv,symlink,pos) SELECT 0,'".$objConn->prepMysql(strtolower($_POST["symlink"]))."',IFNULL(MAX(pos)+1,1) FROM ".PREFIX."_land";
		try
		{
			$objConn->stopCOMMIT();
			if($objConn->i_query($strSQL))
			{
				...
				{
					$strSQL = "INSERT INTO ".PREFIX."_land_text (id_LAND,lan,name,texto,web_title,meta_key,meta_des) VALUES ($iID,'$value','".$objConn->prepMysql($_POST[$value."_name"])."','".$objConn->prepMysql($_POST[$value."_txt"])."','".$objConn->prepMysql($_POST[$value."_web_title"])."','".$objConn->prepMysql($_POST[$value."_meta_key"])."','".$objConn->prepMysql($_POST[$value."_meta_des"])."')";
				...
				header("location:".THISMAINPAGENAME."_edit.php?id_LAND=$iID");
			}
			else
			{
